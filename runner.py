...
decay-pin's arm is (model × compaction on/off) now, growing a pinning toggle in M2. The
violation tally is whatever the deterministic grader already decided per episode; the
runner never re-judges.
...
Run:
    uv run runner.py <label> <model_key_or_slug> [n] [compaction 0|1] [budget_tokens]
...
def main(argv: list[str]) -> int:
    """`uv run runner.py <label> <model_key_or_slug> [n] [compaction 0|1] [budget]`."""
    if len(argv) < 3:
        print(__doc__)
        return 2
    label = argv[1]
