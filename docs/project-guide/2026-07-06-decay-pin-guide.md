...
talk about it. Evidence-anchored; written at the close of v2, refreshed later the same
day after the post-v2 polish (`main` @ `b8b5b14`, tag `v2.0`).*

> **Delta (refresh, 2026-07-06):** three polish PRs landed after the first version of
> this guide — CI now runs all 13 offline suites on every push/PR (#20), the README owns
> the deliberate repo conventions (#21), and the raw run data (459 files + per-run
> manifest) ships on the [v2.0 release](https://github.com/ksdisch/decay-pin/releases/tag/v2.0)
> (#22). Both "fix before showing" flags from the first version are resolved; §5, §7,
> §8 and §10 updated accordingly.
...
On reproducibility: raw run artifacts (`runs/`, per-trial trajectories) are gitignored
because trajectories hold full model conversations — but the complete set behind every
table cell (459 files, with a per-run manifest) is attached to the **v2.0 GitHub
release**, so the claims are auditable without re-spending a token.
...
Twenty-two commits, twenty-one merged PRs, 2026-07-04 → 07-06, closed at tag `v2.0`.
The rhythm was identical every
...
- **Post-v2 polish** (PRs #20–22, tag `v2.0`), same day as close. The two credibility
  gaps this guide's first version flagged were closed: a CI workflow now runs all 13
  offline suites on every push and PR (`.github/workflows/ci.yml` — free by design, no
  key in CI), and the raw run data (459 files + manifest) was published as the v2.0
  release asset. A README "Repo conventions, on purpose" section now states, in-repo,
  the rationale for the deliberate choices (standalone test scripts, flat layout, no
  linter config, data outside git history) — the framings §7 recommends are no longer
  only in this guide.
...
- **Tests gate spend — and CI gates the repo.** 13 offline suites, all free, all green
  before any paid token; new arms added behind default-preserving seams with prior
  suites as regression pins. Since #20 they also run on every push and PR (badge in the
  README), so a reviewer sees green without cloning.
- **Auditable data.** The full raw-run set (459 files + per-run manifest) is a v2.0
  release asset — every table cell can be re-derived without spending a token, paired
  with the per-trial integrity counts in the tables themselves.
- **Deliberate conventions, owned in-repo.** The README's "Repo conventions, on
  purpose" section pre-answers the hygiene questions (why standalone scripts, why flat,
  why no linter, why data outside git) before a reviewer asks them.
...
- ~~No CI~~ / ~~raw data not in the repo~~ — **both resolved** by the post-v2 polish
  (#20, #22). If asked, the story is a good one: "the guide-level audit flagged them
  and they were closed the same day — CI runs the free suites, the release carries the
  data."
- **Hand-rolled test scripts, not pytest; no linter/type-checker config; flat layout
  with `agent.py` at 601 lines.** The README now owns all of this in "Repo conventions,
  on purpose" — in conversation, use the same framing: "each suite is a zero-dependency
  script runnable alone with `uv run`; it's an application, not a package, so nothing
  needed nesting; on a team I'd reach for pytest + ruff. `agent.py` holds the episode
  loop plus all three strategies — the next arm would justify splitting it." Fine when
  owned; junior-smell only if it looks unconsidered, and the README shows it was
  considered.
...
7. The two hygiene fixes (CI, raw-data release) already landed as the post-v2 polish.
   Next in substance: the summarizer-identity question (D17-C) — does a *different*
   model summarizing change rule survival? It's scoped as a fresh brief, not scope
   creep on v2.
...
*(Done since the first version of this guide: CI (#20) and the raw-data release (#22)
— the two fixes it recommended first.)*

1. **v3 brief: summarizer identity (D17-C)** — does a different summarizer model change
   paraphrase survival? New brief, new scope; machinery is cheap behind the existing
   seam, the cost is statistics as always. Days, single-digit dollars. *(Recommended
   first.)*
2. **External write-up** — a post walking the four-row strategy table. Zero spend,
   portfolio value.
...
the paraphrase-survival counts. Each is now owned twice — framed in §7 here and stated
in the README's "Repo conventions, on purpose" section.
...
| `.github/workflows/ci.yml` | CI: runs all 13 offline suites on push + PR (no key needed) |
