"""test_stats.py — offline unit tests for the proportion CIs (ported from forge-gap).

No network, no model, no pytest — run with `uv run test_stats.py` (hand-rolled check style,
the ported forge-gap standard). These intervals are the project's *ruler*: if the math is
off, every floor/gap/restoration claim downstream is wrong. So we pin the functions to
hand-computed reference values (95%, z = 1.96) and to the edge cases that break the naive
±std approach — especially k=0, where our clean floor lives.
...
def test_wilson_edges() -> None:
    ...
    # k=0, n=20 is the M0 clean-floor read: upper ~16%, never "proved 0%".
    lo, hi = wilson(0, 20)
    check("0/20 lower clamps to 0.0", lo == 0.0)
    check("0/20 upper ~ 0.1611 (the M0 floor phrasing)", close(hi, 0.1611))
...
def test_newcombe_overlap_case() -> None:
    print("newcombe — a mild gap (16/20 vs 20/20) straddles 0 -> 'not a result'")
...
def test_newcombe_decay_shape() -> None:
    print("newcombe — the shape M1 hopes to see (floor 0/20 vs decay 7/20)")
    d, lo, hi = newcombe_diff(0, 20, 7, 20)  # 0% floor vs 35% decayed
    check("point estimate d == 0.35", close(d, 0.35))
    check("interval excludes 0 (a claimable gap)", excludes_zero(lo, hi) is True)
    check("interval stays inside [-1, 1]", -1.0 <= lo <= hi <= 1.0)
